...
class BitmaskStrategy(SudokuStrategy):
    def solveSudoku(self, grid: Grid):
        return self.__solveSudokuUsingBitmask(grid, 0, 0)

    def __solveSudokuUsingBitmask(self, grid: Grid, row, col):
        N = 9
        ...
        if grid.nonEmptyCell(row, col):
            return self.__solveSudokuUsingBitmask(grid, row, col + 1)
        for num in range(1, N + 1, 1):
            ...
            if grid.isSafe(row, col, num):
                ...
                if self.__solveSudokuUsingBitmask(grid, row, col + 1):
                    return True
